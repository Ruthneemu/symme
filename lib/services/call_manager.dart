...
class CallManager {
  ...
  void _listenForCallSignals() {
    _callSignalSubscription = FirebaseMessageService.listenForCallSignals().listen(
      (signal) {
    ...
    try {
      switch (type) {
        case 'offer':
          ...
          final callType = signal['callType'] == 'video' ? CallType.video : CallType.audio;
          final call = Call(
            id: callId,
    ...
    try {
      ...
      if (success) {
        final call = Call(
          ...
          callerId: 'current_user', // Will be replaced with actual user ID
          receiverId: receiverSecureId,
          type: callType,
        ...
        _currentCall = call;
        _showCallScreen(call, isIncoming: false);
